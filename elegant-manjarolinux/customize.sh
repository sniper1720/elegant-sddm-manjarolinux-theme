...
RED='\033[0;31m'
NC='\033[0m'
THEME_NAME="elegant-manjarolinux"
...
# Layout Selection
echo
echo "Choose your preferred layout:"
echo "1. Side (Login on right) [Default]"
echo "2. Center (Login in middle)"
read -p "Enter selection [1/2]: " -n 1 -r < /dev/tty
echo
LAYOUT_VAL="Side"
BG_VAL="Assets/background.png"

if [[ $REPLY =~ ^[2]$ ]]; then
    LAYOUT_VAL="Center"
    BG_VAL="Assets/background-center.png"
    echo "Selected: Center Layout"
    
    # Update Metadata Screenshot for KDE Configurator
    if [ -f "$THEME_DIR/metadata.desktop" ]; then
        sed -i 's|Screenshot=Screenshots/side.png|Screenshot=Screenshots/center.png|' "$THEME_DIR/metadata.desktop"
    fi
else
    echo "Selected: Side Layout"
     if [ -f "$THEME_DIR/metadata.desktop" ]; then
        sed -i 's|Screenshot=Screenshots/center.png|Screenshot=Screenshots/side.png|' "$THEME_DIR/metadata.desktop"
    fi
fi

# Write Layout & Background to theme.conf.user immediately
...
if grep -q "layout=" "$CONFIG_FILE" 2>/dev/null; then
    sed -i "s|^layout=.*|layout=$LAYOUT_VAL|" "$CONFIG_FILE"
else
    echo "layout=$LAYOUT_VAL" >> "$CONFIG_FILE"
fi

if grep -q "background=" "$CONFIG_FILE" 2>/dev/null; then
     sed -i "s|^background=.*|background=$BG_VAL|" "$CONFIG_FILE"
else
     echo "background=$BG_VAL" >> "$CONFIG_FILE"
fi
