...
REPO_URL="https://github.com/sniper1720/elegant-sddm-manjarolinux-theme.git"
THEME_NAME="elegant-manjarolinux"
THEME_DIR="/usr/share/sddm/themes/$THEME_NAME"

echo -e "${GREEN}Installing Elegant Manjaro Linux SDDM Theme...${NC}"
...
if git clone "$REPO_URL" "$TEMP_DIR"; then
    ...
    echo "1. Side (Classic, Login on right) [Default]"
    echo "2. Center (Modern, Login in middle)"
    read -p "Enter selection [1/2]: " -n 1 -r < /dev/tty
    echo
    if [[ $REPLY =~ ^[2]$ ]]; then
        echo "Applying Center Layout..."
        echo "[General]
layout=Center
background=Assets/background-center.png" > "$THEME_DIR/theme.conf.user"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        CONFIG_FILE="$CONFIG_DIR/elegant-manjarolinux.conf"
